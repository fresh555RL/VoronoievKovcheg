...
vec2 map(vec3 p){
    ...
    vec2 dPSph = vec2(dSphere(p,vec4(p,0.2)),1);
...
void main(){
    ...
    float onigiri = step(0.0, sin(point.x*25)+sin(point.y*25)+sin(point.z*25));

    colorBase = mix(colorBase, vec3(0.0, 0.0, 0.7), step(point.y, 0.0));
    vec3 even_strong = vec3(0.3);
    vec3 color = colorBase+even_strong*onigiri;
    color = mix(color, vec3(0.83, 0.2, 0.75), step(dPlayer(point, ro),0.0 ));

    FragColor = vec4(color,1.0);
    if(bordermap(point)<0.1){
